...
% Vector Base Amplitude Panning (VBAP) method. The output contains two 
...
function [int_L,int_R] = Interp_VBAP(folder,angles,method)
...
% To compute the VBAP gains and the interpolated IRs, this function takes
...
% Computing the VBAP gains
vbap_A = vbap(complete_dir, angle_pairs_A, inv_mat_A);
...
for n = 1:length(angles_B)
    ...
    % Left channel
    x = repmat((vbap_A(angles_B(n),:)),size(int_L,1),1);
    blank_mat_L_A = x .* ir_L_A;
    blank_vec_L_A = sum(blank_mat_L_A,2);
    int_L(:,2*n) =+ blank_vec_L_A;
    
    % Right channel
    y = repmat((vbap_A(angles_B(n),:)),size(int_R,1),1);
...
% Computing the VBAP gains
vbap_B = vbap(complete_dir, angle_pairs_B, inv_mat_B);
...
for n = 1:length(angles_A)
    ...
    % Left channel
    x = repmat((vbap_B(angles_B(n),:)),size(int_L,1),1);
    blank_mat_L_B = x .* ir_L_B;
    blank_vec_L_B = sum(blank_mat_L_B,2);
    int_L(:,(2*n)-1) =+ blank_vec_L_B;
    
    % Right channel
    y = repmat((vbap_B(angles_B(n),:)),size(int_R,1),1);
