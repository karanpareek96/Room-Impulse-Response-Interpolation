...
function [IR_L,IR_R] = import_IR_mat_data_2(folder)
...
% 30
IR_L (:,2) =+ audio{1,2}; IR_R (:,2) =+ audio{2,2};

% 60
IR_L (:,3) =+ audio{1,4}; IR_R (:,3) =+ audio{2,4};

% 90
IR_L (:,4) =+ audio{1,5}; IR_R (:,4) =+ audio{2,5};

% 120
IR_L (:,5) =+ audio{1,7}; IR_R (:,5) =+ audio{2,7};

% 150
IR_L (:,6) =+ audio{1,9}; IR_R (:,6) =+ audio{2,9};

% 180
IR_L (:,7) =+ audio{1,10}; IR_R (:,7) =+ audio{2,10};

% 210
IR_L (:,8) =+ audio{1,11}; IR_R (:,8) =+ audio{2,11};

% 240
IR_L (:,9) =+ audio{1,13}; IR_R (:,9) =+ audio{2,13};

% 270
IR_L (:,10) =+ audio{1,15}; IR_R (:,10) =+ audio{2,15};

% 300
IR_L (:,11) =+ audio{1,16}; IR_R (:,11) =+ audio{2,16};

% 330
IR_L (:,12) =+ audio{1,18}; IR_R (:,12) =+ audio{2,18};
